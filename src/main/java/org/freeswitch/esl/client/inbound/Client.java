...
public class Client {
    ...
    private final ThreadFactory eslEventNotifierThreadFactory = new ThreadFactory() {
    ...
    private final Executor eventListenerExecutor = new ThreadPoolExecutor(1, 1,
            0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(1024), eslEventNotifierThreadFactory, new ThreadPoolExecutor.AbortPolicy());


    private final ThreadFactory eslBackgroundJobNotifierThreadFactory = new ThreadFactory() {
        AtomicInteger threadNumber = new AtomicInteger(1);
    ...
    private final Executor backgroundJobListenerExecutor = new ThreadPoolExecutor(1, 1,
            0L, TimeUnit.MILLISECONDS,
            new LinkedBlockingQueue<>(1024), eslBackgroundJobNotifierThreadFactory, new ThreadPoolExecutor.AbortPolicy());
    ...
    private static final ThreadFactory bossThreadFactory = new ThreadFactoryBuilder().setNameFormat("boss-pool-%d").build();
    private static final ThreadFactory workerThreadFactory = new ThreadFactoryBuilder().setNameFormat("worker-pool-%d").build();

    private static ExecutorService bossPool = null;
    private static ExecutorService workerPool = null;
    private static NioClientSocketChannelFactory nioClientSocketChannelFactory = null;

    private int bossMaxNumber = 1;
    private int workerMinNumber = Runtime.getRuntime().availableProcessors() * 2;
    private int workerMaxNumber = 16;


    public Client(int bossMaxNum, int workerMaxNum) {
        if (bossMaxNum > 0) {
            bossMaxNumber = bossMaxNum;
        }
        if (workerMaxNum > workerMinNumber) {
            workerMaxNumber = workerMaxNum;
        }
     ...
     * {@link Client#addEventFilter(java.lang.String, java.lang.String)}
     *
